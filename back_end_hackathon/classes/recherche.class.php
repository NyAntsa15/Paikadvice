<?php
    class Recherche extends Dbh{
        private function forum_search($recherche) {
            $sql = "SELECT Texte FROM Forum WHERE Texte LIKE '%?%'";
        ...
        private function news_search($recherche) {
            $sql = "SELECT texte FROM Journal WHERE texte LIKE '%?%'";
            $stmt = $this->dbconnect()->prepare($sql);
            $stmt->execute([$recherche]);
            $names = $stmt->fetchAll();
        ...
        private function vulgarisateurs_search($recherche) {
            $sql = "SELECT Nom, Prenom FROM Vulg WHERE Nom OR Prenom LIKE '%?%'";
            $stmt = $this->dbconnect()->prepare($sql);
            $stmt->execute([$recherche]);
            $names = $stmt->fetchAll();
        ...
        private function agriculteurs_search($recherche) {
            $sql = "SELECT Nom, Prenom FROM Agri WHERE Nom OR Prenom LIKE '%?%'";
            $stmt = $this->dbconnect()->prepare($sql);
            $stmt->execute([$recherche]);
            $names = $stmt->fetchAll();
        ...
        protected function all_search($recherche) {
